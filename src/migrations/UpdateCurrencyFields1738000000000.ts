...
export class UpdateCurrencyFields1738000000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    const [hasBillingInvoice, hasSchoolBillingPlan, hasFeePayment, hasFeeStructure] = await Promise.all([
      queryRunner.hasTable('billing_invoice'),
      queryRunner.hasTable('school_billing_plan'),
      queryRunner.hasTable('fee_payment'),
      queryRunner.hasTable('fee_structure'),
    ]);
    ...
    const feePaymentTable = hasFeePayment ? await queryRunner.getTable('fee_payment') : undefined;
    ...
    const feeStructureTable = hasFeeStructure ? await queryRunner.getTable('fee_structure') : undefined;
